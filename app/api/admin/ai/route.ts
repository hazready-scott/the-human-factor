...
The article should be 800-1200 words, well-structured with clear sections, and include practical insights. Write for an audience of organizational leaders considering system improvement and AI integration.`
        break
...
Article content: ${context.content?.substring(0, 2000)}`
      ...
      case 'outline':
        userPrompt = `Create a detailed article outline about: "${context.topic}"

Format as HTML using <h2> for main sections and <ul><li> for sub-points under each section. Include 4-6 main sections with 2-4 sub-points each. The outline should be ready to expand into a full article.`
        break
    ...
    // For SEO action, parse the JSON
    if (action === 'seo') {
      try {
        const seoData = JSON.parse(result)
        return NextResponse.json({ result: seoData, type: 'json' })
      } catch {
