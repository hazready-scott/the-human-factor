...
export default function ContactDetailPage() {
  ...
  if (!contact) return <div className="text-slate-500 text-center py-20">Loading...</div>

  return (
    <div className="max-w-4xl">
      <Link href="/admin/contacts" className="flex items-center gap-2 text-sm text-slate-500 hover:text-slate-300 mb-6 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="admin-card">
          ...
          <dl className="space-y-3 text-sm">
            <div className="flex justify-between"><dt className="text-slate-500">Organization</dt><dd className="font-medium text-slate-300">{contact.organization || '—'}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-500">Phone</dt><dd className="font-medium text-slate-300">{contact.phone || '—'}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-500">Source</dt><dd className="font-medium text-slate-300 capitalize">{contact.source.replace('_', ' ')}</dd></div>
            <div className="flex justify-between"><dt className="text-slate-500">Created</dt><dd className="font-medium text-slate-300">{new Date(contact.created_at).toLocaleString()}</dd></div>
            {contact.utm_source && <div className="flex justify-between"><dt className="text-slate-500">UTM Source</dt><dd className="font-medium text-slate-300">{contact.utm_source}</dd></div>}
          </dl>
          {contact.message && (
            <div className="mt-4 pt-4 border-t border-white/5">
              <h3 className="text-sm font-medium text-slate-500 mb-2">Message</h3>
              <p className="text-sm text-slate-300 leading-relaxed">{contact.message}</p>
            </div>
          )}
        </div>
        ...
        {/* Status & Notes */}
        <div className="admin-card">
          <h2 className="font-semibold text-white mb-4">CRM Status</h2>
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">Status</label>
              <select value={status} onChange={e => setStatus(e.target.value)} className="admin-input capitalize">
                {STATUSES.map(s => <option key={s} value={s} className="bg-[#111827]">{s}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-400 mb-1">Notes</label>
              <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={6} className="admin-input resize-none" placeholder="Internal notes..." />
            </div>
          </div>
        </div>
